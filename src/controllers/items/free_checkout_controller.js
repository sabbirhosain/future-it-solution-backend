...
import FreeCheckoutModel from "../../models/items/free_checkout_model.js";
...
export const create = async (req, res) => {
    try {
        ...
        // check exist data
        const findItem = await FreeItemsModel.findById(item_id);
        ...
        // Save to DB
        const result = await new FreeCheckoutModel({
...
export const show = async (req, res) => {
    try {
        ...
        const result = await FreeCheckoutModel.find(dataFilter)
            .sort({ createdAt: -1 })
            .limit(limit)
            .skip((page - 1) * limit)

        const count = await FreeCheckoutModel.find(dataFilter).countDocuments();
...
export const single = async (req, res) => {
    try {
        ...
        const result = await FreeCheckoutModel.findById(id);
...
export const update = async (req, res) => {
    try {
        ...
        // check exist data
        const findItem = await FreeCheckoutModel.findById(id);
        if (!findItem) { return res.json({ success: false, message: "Item not found" }) }
        ...
        // update
        const result = await FreeCheckoutModel.findByIdAndUpdate(id, {
        ...
        // Update total_sold in FreeItemsModel if needed
        if (totalSoldUpdate !== 0 && mongoose.Types.ObjectId.isValid(result.item_id)) {
            await FreeItemsModel.findByIdAndUpdate(result.item_id, { $inc: { total_sold: totalSoldUpdate } });
        }
...
export const destroy = async (req, res) => {
    try {
        ...
        // check exist data
        const findOne = await FreeCheckoutModel.findById(id);
        if (!findOne) {
            return res.json({ message: "Item not found" });
        }

        const result = await FreeCheckoutModel.findByIdAndDelete(id);
